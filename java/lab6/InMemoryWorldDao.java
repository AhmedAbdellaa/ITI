...
public class InMemoryWorldDao implements WorldDao {
	...
	public static void main(String args[]) {
		...
		//allCountries.values().stream().flatMap(country->country.getCities().stream()).forEach(r->System.out.println(r.getClass().getName())); return city object
		//name : Tegucigalpain country : HND
		//name : San Pedro Sulain country : HND
		//Sname : La Ceibain country : HND
		//one to many convert 
		
		//allCountries.values().stream().map(country->country.getCities()).forEach(r->System.out.println(r.getClass().getName()));
		//[name : Tegucigalpain country : HND, name : San Pedro Sulain country : HND, name : La Ceibain country : HND]
		//one to one convert 
		/*
		System.out.println(allCountries.values().stream()
		...
		*/
		...
		String nam = country.getName();
		country.getCities().stream()
		.max(Comparator.comparing(City::getPopulation))
		.ifPresent(e -> System.out.printf("country : %-35s| city : %-25s| population : %-10s|\n" , nam ,e.getName(), e.getPopulation()));
		}) ;
		
		/*
		allCountries.values().stream()
		.map((country)->country.getCities().stream().max(Comparator.comparing(City::getPopulation)))
		.forEach(e->{if(e.isPresent())
		System.out.println(e.get());});
		*/
		/////////////////////////////////////////////////////////
		////////////////////////////////Find the most populated city of each continent////////////////////////////
		System.out.println("//////////////////////////////////////////////////////////");
		System.out.println("//////Find the most populated city of each continent//////");
		System.out.println("//////////////////////////////////////////////////////////");
		allContinents.forEach(
		cont->
			//String nam = cont ;
			allCountries.values().stream()
			.filter(cou ->cou.getContinent() == cont )
			.flatMap((country)->country.getCities().stream())
			.max(Comparator.comparing(City::getPopulation))
			.ifPresent(e->System.out.printf(" Continent :%-15s| city: %s \n" ,cont , e ))
		...
		System.out.println("///////////////////////////////////////////////////");
		allCities.values().stream().filter(ci-> allCountries.get(ci.getCountryCode()).getCapital() == ci.getId())
		.max(Comparator.comparing(City::getPopulation))
